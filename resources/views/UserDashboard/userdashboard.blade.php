		...
		<div class="w-full bg-gray-50 p-6 rounded-xl border border-gray-100">
			<div class="flex flex-col lg:flex-row gap-6">
				<div class="flex-grow min-w-0 relative group">
					<div x-show="loading" class="flex justify-center items-center h-48 w-full">
				...
				<div class="lg:w-64 flex-shrink-0 bg-white rounded-xl p-5 border border-gray-200 text-center shadow-sm flex flex-col justify-center h-full min-h-[180px]">
					<div class="flex items-center gap-3 mb-3 justify-center">
						<div class="relative">
							<img
								...
								?: 'https://api.dicebear.com/7.x/avataaars/svg?seed=' . auth()->user()->id }}"
								alt="{{ auth()->user()->name }}"
					...
					<div class="space-y-1 text-left bg-gray-50 p-2 rounded-lg text-[10px]">
						<p class="flex justify-between text-gray-500 font-medium">
							Plan:
					...
					<a href='/market-calls' class="w-full bg-[#0939a4] hover:bg-blue-700 text-white text-[10px] font-bold py-2 rounded-lg mt-3 transition-colors">
					View Market Calls
					</a>
				</div>
			</div>
				...
				<template x-for="(stock, index) in stocks" :key="stock.id">
					<div x-show="(activeTab === 1 && index < 4) || (activeTab === 2 && index >= 4)"
						...
						<div class="flex justify-between text-center border-t border-gray-50 pt-4">
							<div>
								<p class="text-gray-400 text-[12px] mb-1">Entry</p>
								<p class="text-[#00c853] font-bold text-sm" x-text="'₹' + stock.entry"></p>
							</div>
							<div>
								<p class="text-gray-400 text-[12px] mb-1">Target</p>
								<p class="text-black font-bold text-sm" x-text="'₹' + stock.target"></p>
							</div>
							<div>
								<p class="text-gray-400 text-[12px] mb-1">Stop-Loss</p>
								<p class="text-[#ff3d00] font-bold text-sm" x-text="'₹' + stock.sl"></p>
							</div>
						</div>
					</div>
				</template>
		...
		<div class="flex flex-col">
			...
			<div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden h-full">
				...
				<div class="p-4">
					<table class="w-full">
						<thead>
							<tr class="text-gray-400 text-[11px] font-bold uppercase tracking-tight">
								<th class="text-left pb-4">Symbol</th>
								<th class="text-center pb-4" x-text="timeFilter + ' change'"></th>
								<th class="text-right pb-4">LTP</th>
							</tr>
						</thead>
